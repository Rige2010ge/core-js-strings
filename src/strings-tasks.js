...
function concatenateStrings(value1, value2) {
  const strConcat = '';
  return strConcat.concat(value1, value2);
}
...
function removeFirstOccurrences(str, value) {
  ...
  return (
    str.slice(0, index !== -1 ? index : str.length) +
    str.slice(index !== -1 ? index + value.length : str.length + 1)
  );
}
...
function sumOfCodes(str) {
  if (str) {
    return str
      .split('')
      .reduce((acc, element) => acc + element.charCodeAt(0), 0);
  }
  return 0;
}
...
function isPalindrome(str) {
  const reg = /\w/gi;
  const regString = str.match(reg).join('').toLowerCase();
  const revString = str.match(reg).reverse().join('').toLowerCase();
  return regString === revString;
}
